...
function processScriptTags(html: string): string {
  ...
  return html.replace(/<script([^>]*?)>([\s\S]*?)<\/script>/gi, (_match, attributes, content) => {
...
export function preprocessContent(content: string, chatSessionId: string): string {
  ...
  while ((match = iframeStartRegex.exec(content)) !== null) {
    ...
    if (tagEnd === -1) {
      ...
      console.log(
        `Detected incomplete iframe during streaming (${estimatedLength} chars), replacing with loading indicator`
      )
    ...
    if (isSelfClosing) {
      ...
      const processed = iframeContent.replace(
        ...
        (_match, iframeTag, beforeSrcdoc, srcdocStart, _quote, srcdocContent, srcdocEnd, afterSrcdoc) => {
          ...
          const cleanedSrcdoc = withAutoResize.replace(/\n/g, '').replace(/\r/g, '').replace(/\s+/g, ' ').trim()
    ...
    const iframeContent = iframeEnd === -1 ? content.substring(iframeStart) : content.substring(iframeStart, iframeEnd)
