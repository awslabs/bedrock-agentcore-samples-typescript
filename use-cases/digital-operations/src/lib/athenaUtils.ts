  ...
  async function poll(retries = 0): Promise<AthenaQueryResult<T>> {
    ...
    try {
      ...
      if (status === 'SUCCEEDED' && statusResult.data?.data) {
        const dataString =
          typeof statusResult.data.data === 'string' ? statusResult.data.data : JSON.stringify(statusResult.data.data)
        ...
        throw new AthenaQueryError(error || `Query ${status.toLowerCase()}: ${status}`, queryExecutionId, status)
      } else if (error) {
...
export async function executeAthenaQuery<T = unknown>(
  ...
  try {
    ...
    if (!queryResult.data?.queryExecutionId) {
      throw new AthenaQueryError('Failed to start query: No query execution ID returned', undefined, 'FAILED')
    }
    ...
    throw new AthenaQueryError(error instanceof Error ? error.message : 'Failed to execute query', undefined, 'ERROR')
  }
}
