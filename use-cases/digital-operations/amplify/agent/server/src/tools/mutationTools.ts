...
const createMapLayerTool = {
  ...
  config: {
    ...
    description: 'Create a new query-based map layer for the current chat session. The layer executes an Athena SQL query to generate GeoJSON data dynamically. The query will be validated before creating the layer. The chat session ID is automatically provided.',
    inputSchema: z.object({
      name: z.string().describe('Display name for the layer (e.g., "High Production Wells", "Pipeline Network")'),
      type: z.enum(['point', 'line', 'polygon', 'heatmap', 'geojson']).describe('Type of geometry: point (markers), line (pipelines), polygon (boundaries), heatmap (density), or geojson (mixed)'),
      athenaQuery: z.string().describe('SQL query to generate GeoJSON data from Athena. Example: "SELECT id, name, latitude, longitude FROM wells WHERE status = \'Active\'"'),
      athenaDatabase: z.string().describe('Athena database name for the query. Example: "upstream"'),
      geoJsonMapping: z.object({
        ...
        coordinatesField: z.string().optional().describe('Column name containing coordinate array (required for LineString/Polygon)'),
        propertyFields: z.array(z.string()).optional().describe('Column names to include as feature properties. If omitted, all columns except coordinate fields are included.')
      }).describe('Mapping configuration for converting query results to GeoJSON'),
      queryRefreshInterval: z.number().optional().describe('Minutes between automatic query refresh (0 = manual only). Default: 0'),
      style: z.object({
        ...
        colorScale: z.object({
          type: z.enum(['linear', 'categorical', 'step']).describe('Type of color scale: linear (gradient), categorical (discrete categories), step (ranges)'),
          property: z.string().describe('Property name from query results to use for coloring'),
          stops: z.array(z.tuple([z.union([z.number(), z.string()]), z.string()])).optional().describe('Array of [value, color] pairs for linear/step scales. Example: [[0, "#blue"], [50, "#yellow"], [100, "#red"]]'),
          categories: z.record(z.string()).optional().describe('Object mapping category values to colors. Example: {"Active": "#green", "Inactive": "#red"}'),
          defaultColor: z.string().optional().describe('Fallback color for values not in scale')
        }).optional().describe('Data-driven color scale configuration'),
        radiusScale: z.object({
        ...
        }).optional().describe('Data-driven radius scale for points'),
        tooltip: z.object({
          ...
          })).optional().describe('Fields to display in tooltip')
        }).optional().describe('Tooltip configuration')
      }).optional().describe('Style configuration with optional data-driven styling'),
      ...
      source: z.string().optional().describe('Data source identifier (e.g., "athena-query", "user-input", "ai-analysis")')
    })
  },
...
const updateMapLayerTool = {
  ...
  config: {
    ...
    description: 'Update an existing map layer. Use this to modify layer query, style, visibility, or other properties.',
    inputSchema: z.object({
      ...
      }).optional().describe('Updated GeoJSON mapping configuration'),
      ...
      }).optional().describe('Updated style configuration with optional data-driven styling'),
      tooltip: z.object({
        title: z.string().optional().describe('Property name to use as tooltip title (defaults to "name")'),
        fields: z.array(z.object({
          property: z.string().describe('Property name from query results'),
...
const updateActionItemTool = {
  ...
  config: {
    ...
    description: 'Update the status of an action item (approve, reject, or defer). Use this when the user makes decisions about pending actions.',
    inputSchema: z.object({
...
const updateWorkoverJobTool = {
  ...
  config: {
    ...
    inputSchema: z.object({
      ...
      status: z.enum(['queued', 'inProgress', 'completed', 'delayed']).optional().describe('New status for the workover job'),
